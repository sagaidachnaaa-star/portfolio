...
import { anchorSegmentToShadowScope, legacyPathIsRootAnchored, segmentWalk, SHADOW_BOUNDARY_DELIMITER, } from './helpers/shadow';
/**
...
export function createSelectorEngine(initialConfig, options) {
    ...
    var config = normalizeResolvedConfig(initialConfig);
     ...
     */
    var legacyTree = function (target) { return safeLegacyCssPath(target, logger); };
    /**
     * ENGINE algorithm: the strategy chain, then the hardened fallback, both
     * scoped to the tree's own root.
     */
    var engineTree = function (target, scope) {
        var _a;
        return (_a = runOrchestrator(target, config, { strategies: options.strategies, scope: scope, logger: logger })) !== null && _a !== void 0 ? _a : fallbackCssPath(target, config, { scope: scope });
    };
     ...
     */
    var legacyTreeInShadow = function (target, scope) {
        return anchorSegmentToShadowScope(legacyTree(target, scope), scope, legacyPathIsRootAnchored(target));
    };
    /**
     ...
     */
    function pierce(el, perTree) {
        var _a = segmentWalk(el, config.maxShadowDomDepth), segments = _a.segments, truncated = _a.truncated;
        if (truncated) {
            logger === null || logger === void 0 ? void 0 : logger.debug("@amplitude/element-selector: target is nested deeper than maxShadowDomDepth (".concat(config.maxShadowDomDepth, ") \u2014 emitting a best-effort selector for the outermost in-budget shadow host"));
        }
        return segments.map(function (_a) {
            var target = _a.target, root = _a.root;
            return perTree(target, root);
        }).join(SHADOW_BOUNDARY_DELIMITER);
    }
    return {
        generate: function (el) {
            ...
            try {
                if (!config.shadowDomEnabled) {
                    var perTree = config.enabled ? engineTree : legacyTree;
                    return perTree(el, (_b = (_a = options.scope) !== null && _a !== void 0 ? _a : el.ownerDocument) !== null && _b !== void 0 ? _b : document);
                }
                return pierce(el, config.enabled ? engineTree : legacyTreeInShadow);
            }
            catch (e) {
                var message = e instanceof Error ? e.message : String(e);
                logger === null || logger === void 0 ? void 0 : logger.warn("@amplitude/element-selector: selector generation threw \u2014 falling back to legacy cssPath: ".concat(message));
