...
var MID_HEIGHT_LINE_TOLERANCE_PX = 1;
var getVisualParent = function (element) {
    var _a;
    return element.assignedSlot || element.parentElement || ((_a = element.parentNode) === null || _a === void 0 ? void 0 : _a.host) || null;
};
/**
 ...
 */
var getClippingParent = function (element, position) {
    return (position === 'fixed' || position === 'absolute') && element instanceof HTMLElement
        ? element.offsetParent
        : getVisualParent(element);
};
var isMidHeightLineVisible = function (element) {
    var globalScope = (0, analytics_core_1.getGlobalScope)();
    /* istanbul ignore next -- trackExposure is only installed in a browser */
    var viewportHeight = globalScope.document.documentElement.clientHeight || globalScope.innerHeight;
    /* istanbul ignore next -- trackExposure is only installed in a browser */
    var viewportWidth = globalScope.document.documentElement.clientWidth || globalScope.innerWidth;
    var rect = element.getBoundingClientRect();
    var midHeightLine = rect.top + rect.height * constants_1.EXPOSURE_VIEWED_THRESHOLD;
    ...
    if (midHeightLine < -MID_HEIGHT_LINE_TOLERANCE_PX ||
        midHeightLine > viewportHeight + MID_HEIGHT_LINE_TOLERANCE_PX ||
        visibleLeft >= visibleRight ||
        rect.width <= 0) {
        return false;
    }
    var ancestor = getClippingParent(element, globalScope.getComputedStyle(element).position);
    while (ancestor) {
        /* istanbul ignore next -- trackExposure is only installed in a browser */
        var style = globalScope.getComputedStyle(ancestor);
        // Browsers return "visible" by default; jsdom returns an empty string.
        var clipsY = style.overflowY && style.overflowY !== 'visible';
        var clipsX = style.overflowX && style.overflowX !== 'visible';
        if (clipsY || clipsX) {
...
function trackExposure(_a) {
    ...
    var intersectingElements = new Set();
    var cancelExposure = function (element) {
    ...
    var updateExposure = function (element) {
    ...
    var exposureObservable = allObservables.exposureObservable, scrollObservable = allObservables.scrollObservable;
    ...
    var clearPageViewState = function () {
