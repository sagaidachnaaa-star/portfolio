...
var shadow_mode_1 = require("./shadow-mode");
var hasMaskedAncestorLight = function (element) { return element.closest("[".concat(analytics_core_1.TEXT_MASK_ATTRIBUTE, "]")) !== null; };
var hasMaskedAncestorInShadow = function (element, shadow) {
    var _a, _b;
    if ((0, hierarchy_1.getAncestors)(element, shadow).some(function (ancestor) { return ancestor.hasAttribute(analytics_core_1.TEXT_MASK_ATTRIBUTE); })) {
        return true;
    }
    // `getAncestors` stops before `<html>`; preserve the document-element check.
    return (_b = (_a = document.documentElement) === null || _a === void 0 ? void 0 : _a.hasAttribute(analytics_core_1.TEXT_MASK_ATTRIBUTE)) !== null && _b !== void 0 ? _b : false;
};
/**
...
var DataExtractor = /** @class */ (function () {
    function DataExtractor(options, context) {
        ...
        this.getHierarchy = function (element) {
            ...
            var shadow = _this.getShadowMode();
            var ancestors = (0, hierarchy_1.getAncestors)(element, shadow);
            ...
            hierarchy = ancestors.map(function (el) { var _a; return (0, hierarchy_1.getElementProperties)(el, (_a = elementToAttributesToMaskMap.get(el)) !== null && _a !== void 0 ? _a : new Set(), shadow); });
        ...
        this.updateSelectorConfig = function (remote, logger) {
            ...
            var hasEnabled = typeof remote.enabled === 'boolean';
            var hasShadow = typeof remote.shadowDomEnabled === 'boolean';
            if (!hasEnabled && !hasShadow) {
                logger === null || logger === void 0 ? void 0 : logger.debug('@amplitude/element-selector: ignoring remote-config delivery without an explicit `enabled` or `shadowDomEnabled` flag — keeping current engine state.');
                return;
            }
            var resolved = (0, element_selector_1.resolveSelectorConfig)(remote, logger);
            var prevEnabled = _this.selectorEngine.getConfig().enabled;
            var prevShadow = _this.shadowGate.get().enabled;
            var mode = _this.shadowGate.arm((0, shadow_mode_1.shadowModeFromConfig)(resolved));
            _this.selectorEngine.updateConfig(tslib_1.__assign(tslib_1.__assign(tslib_1.__assign({}, (hasEnabled ? resolved : _this.selectorEngine.getConfig())), { shadowDomEnabled: mode.enabled }), (mode.enabled && { maxShadowDomDepth: mode.maxDepth })));
            if (hasEnabled && prevEnabled !== resolved.enabled) {
                logger === null || logger === void 0 ? void 0 : logger.debug(resolved.enabled
            ...
            if (!prevShadow && mode.enabled) {
                (_a = _this.diagnosticsClient) === null || _a === void 0 ? void 0 : _a.setTag('plugin.autocapture.shadowDom', "enabled:".concat(mode.maxDepth));
            }
        };
        /**
         ...
         */
        this.getShadowMode = function () {
            return _this.shadowGate.get();
        };
        ...
        isCapturingCursorPointer) {
            ...
            try {
                if ((0, helpers_1.isElementBasedEvent)(baseEvent) && baseEvent.event.target !== null) {
                    ...
                    var shadow = _this.getShadowMode();
                    var eventTarget = (0, helpers_1.resolveEventTarget)(baseEvent.event, shadow);
                    if (isCapturingCursorPointer) {
                        var isCursorPointer = (0, helpers_1.isElementPointerCursor)(eventTarget, baseEvent.type);
                        if (isCursorPointer) {
                            baseEvent.closestTrackedAncestor = eventTarget;
                    ...
                    var closestTrackedAncestor = (0, helpers_1.getClosestElement)(eventTarget, selectorAllowlist, shadow);
         ...
         */
        this.hasMaskedAncestor = function (element) {
            var shadow = _this.getShadowMode();
        ...
        this.getText = function (element) {
            ...
            var hasMaskAttribute = _this.hasMaskedAncestor(element);
            if (hasMaskAttribute) {
        ...
        this.shadowGate = (0, shadow_mode_1.getSharedShadowGate)();
